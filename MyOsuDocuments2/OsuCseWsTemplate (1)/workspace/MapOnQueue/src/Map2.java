...
public class Map2<K, V> extends MapSecondary<K, V> {
    ...
    private static <K, V> void moveToFront(Queue<Pair<K, V>> q, K key) {
        ...
        // while index is less that queues length
        // check for the key in the queue
        // if true stop
        // if not rotate the queue by one
        // if key not found iterate until index == q.length

        while (index < q.length() && !found) {
            Map.Pair<K, V> sameAsx = q.front();
            if (sameAsx.key().equals(key)) {
                found = true;
            } else {
                q.rotate(1);
                index++;
    ...
    public final void add(K key, V value) {
        ...
        MapSecondary.SimplePair<K, V> s = new MapSecondary.SimplePair<>(key, value);

        this.pairsQueue.enqueue(s);
    ...
    public final boolean hasKey(K key) {
        ...
        if (this.pairsQueue.length() > 0) {
            ...
            Map.Pair<K, V> pairFromKey = this.pairsQueue.front();

            result = pairFromKey.key().equals(key);
        }
        return result;
    }
